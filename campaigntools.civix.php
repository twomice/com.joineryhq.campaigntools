...
function _campaigntools_civix_civicrm_managed(&$entities) {
  ...
  foreach ($mgdFiles as $file) {
    ...
    foreach ($es as $e) {
      if (empty($e['module'])) {
        $e['module'] = 'com.joineryhq.campaigntools';
      }
      $entities[] = $e;
    }
  }
...
function _campaigntools_civix_civicrm_caseTypes(&$caseTypes) {
  ...
  foreach (_campaigntools_civix_glob(__DIR__ . '/xml/case/*.xml') as $file) {
    ...
    $caseTypes[$name] = array(
      'module' => 'com.joineryhq.campaigntools',
      'name' => $name,
      'file' => $file,
    );
  }
...
function _campaigntools_civix_civicrm_angularModules(&$angularModules) {
  ...
  foreach ($files as $file) {
    ...
    $module = include $file;
    if (empty($module['ext'])) {
      $module['ext'] = 'com.joineryhq.campaigntools';
    }
 ...
 */
function _campaigntools_civix_insert_navigation_menu(&$menu, $path, $item) {
  ...
  if (empty($path)) {
    ...
    foreach ($menu as $key => &$entry) {
      if ($entry['attributes']['name'] == $first) {
        ...
        $found = _campaigntools_civix_insert_navigation_menu($entry['child'], implode('/', $path), $item, $key);
      }
    }
    return $found;
  }
 ...
 */
function _campaigntools_civix_navigationMenu(&$nodes) {
  if (!is_callable(array('CRM_Core_BAO_Navigation', 'fixNavigationMenu'))) {
    _campaigntools_civix_fixNavigationMenu($nodes);
  }
}
 ...
 * missing them.
 */
function _campaigntools_civix_fixNavigationMenu(&$nodes) {
  $maxNavID = 1;
  ...
  _campaigntools_civix_fixNavigationMenuItems($nodes, $maxNavID, NULL);
}

function _campaigntools_civix_fixNavigationMenuItems(&$nodes, &$maxNavID, $parentID) {
